...
namespace LoggingKata
{
    class Program
    {
        ...
        static void Main(string[] args)
        {
            ...
            if(lines.Length == 0)
			{
                logger.LogError("0 lines to read");
			}

            if(lines.Length == 1)
			{
                logger.LogWarning("Not enough lines");
			}
            ...
            for (int i = 0; i < locations.Length; i++)
			{
				...
				{
                    var cordB = new GeoCoordinate(locations[j].Location.Latitude, locations[j].Location.Longitude);
                    if(cordA.GetDistanceTo(cordB) > distance)
					{
                        ...
                        logger.LogInfo($"current highest distance is{distance}");

                    }
                }
			}
            logger.LogInfo("Comparison Complete");
